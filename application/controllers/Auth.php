...
class Auth extends CI_Controller {
	...
	public function cek()
	{
		...
		$pengguna = $this->auth->cek_peng(['username' => $username]);
		if ($pengguna[0]->id_karyawan != null) {
			$ceklvus = $this->auth->ceklevel($pengguna[0]->id_level_user);
			if ($ceklvus[0]->level_user == 'Broker') {
			...
			if ($ceklvus[0]->level_user == 'Asuransi') {
					...
					if (password_verify($password, $dataz[0]->password)) {
						...
						if ($prv != 0) {
							$array = array(
								...
								'nm_karyawan' => $dataz[0]->nama_asuransi,
								'id_jabatan' => 0,
								'id_bagian' => 0,
			...
			if ($ceklvus[0]->level_user == 'Tertanggung') {
				$cektr = $this->auth->cektrt(['usern'=>$username,'flg'=>getdbtable($pengguna[0]->flag_table)]);
				if ($cektr->num_rows() > 0) {
					$datab = $cektr->result();
			...
			// if ($ceklvus[0]->level_user == 'Pengguna Tertanggung') {
			// 	// code...
			// }
		} else { // pas admin
			if ($pengguna[0]->id_level_otorisasi == 0) {
				if (password_verify($password, $pengguna[0]->password)) {
					$array = array(
						'sesi_id' => $pengguna[0]->id_user,
						'username' => $pengguna[0]->username,
						'nm_karyawan' => $pengguna[0]->username,
						'id_jabatan' => 0,
						'id_bagian' => 0,
	...
	// 	$password	= $this->security->xss_clean($this->input->post('password'));
	//
	// 	$iskarywn = $this->auth->cek_peng(['username' => $username]);
	...
	public function generate_pass()
	{
		echo "diana";
		echo "<br>";
		echo password_hash("diana123", PASSWORD_DEFAULT);
		echo "<br>";
		echo "haikal";
		echo "<br>";
		echo password_hash("haikal123", PASSWORD_DEFAULT);
		echo "<br>";
		echo "hisyam";
		echo "<br>";
		echo password_hash("hisyam123", PASSWORD_DEFAULT);
		echo "<br>";
		echo "ifan";
		echo "<br>";
		echo password_hash("ifan123", PASSWORD_DEFAULT);
	}
