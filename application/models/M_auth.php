...
class M_auth extends CI_Model {
  ...
  public function whennotkarywn($value) {
    $this->db->select('m_user.id_user,m_user.username,m_user.password,m_asuransi.nama_asuransi,level_otorisasi.id_level_otorisasi');
		$this->db->join('m_asuransi','m_user.id_karyawan = m_asuransi.id_asuransi');
    $this->db->join('level_otorisasi','m_user.id_level_otorisasi = level_otorisasi.id_level_otorisasi');
    $this->db->where($value);
    return $this->db->get('m_user');
  }
  ...
  public function cektrt($lisn)
  {
    ...
    $this->db->select($lisn['flg'].'.'.$fnme[0]['nmny'].' as nama,m_user.id_user,m_user.username,m_user.password,level_otorisasi.id_level_otorisasi');
