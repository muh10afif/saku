...
<?php if ($role['read'] == true || $role == null): ?>
  <div class="row">
              ...
              <table id="datatable_ro" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                <thead>
                  <tr>
                    ...
                    <th style="width:13%;">Kode Agent</th>
    ...
    <div class="col-md-5">
      <div class="card shadow">
        <form id="colectagnt" method="post">
          <div class="card-body">
            ...
            <div class="form-group">
              ...
              <input type="text" name="kdag" id="kdag" class="form-control" required placeholder="Kode Agent" readonly/>
            </div>
            <div class="form-group">
              <label>Nama<b style="color:red;">*</b></label>
              <input type="hidden" name="idag" id="idag" value="">
              <input type="text" name="nmag" id="nmag" class="form-control" required placeholder="Nama"/>
            </div>
            <div class="form-group">
              <label>No. Telepon<b style="color:red;">*</b></label>
              <input type="text" name="tlag" id="tlag" class="form-control numeric" required placeholder="Telepon"/>
            </div>
            ...
            <div class="form-group">
              ...
              <textarea name="alag" id="alag" class="form-control" required placeholder="Alamat" rows="8" cols="80"></textarea>
            </div>
...
<script type="text/javascript">
  ...
  function getkode() {
    $.ajax({
      type:"GET",
      url:"<?php echo base_url(); ?>agent/agent_kode",
  ...
  $(document).ready(function () {
    ...
    table_agent_ro = $('#datatable_ro').DataTable({
      "responsive": true,
      "processing" : true,
      "serverSide" : true,
      "order" : [],
      "ajax" : {
        "url" : "<?php echo base_url(); ?>agent/ajaxdata/"+act,
        "type" : "POST",
        "data" : { "tipen":'t' }
      },
      "columnDefs" : [{
    ...
    table_agent_ru = $('#datatable_ru').DataTable({
      "responsive": true,
      "processing" : true,
      "serverSide" : true,
      "order" : [],
      ...
      swal({
        ...
        if (result.value) {
          if (agid == '') {
            ...
            $.ajax({
              type:"POST",
              url:"<?php echo base_url(); ?>agent/edit/"+agid,
              beforeSend : function () {
                swal({
                  title  : 'Menunggu',
                  html   : 'Memproses Data',
          ...
          table_agent_ro.ajax.reload();
          table_agent_ru.ajax.reload();
          table_agent_ro.columns.adjust().draw();
          table_agent_ru.columns.adjust().draw();
        } else if (result.dismiss === swal.DismissReason.cancel) {
          swal({
            title               : "Batal",
            text                : 'Anda membatalkan Penginputan Data Agent',
            buttonsStyling      : false,
            confirmButtonClass  : "btn btn-primary",
            type                : 'error',
            showConfirmButton   : false,
  ...
  function ubahubah(id) {
    ...
    $.ajax({
      type:"GET",
      url:"<?php echo base_url(); ?>agent/show/"+id,
      success  : function (data) {
        var hss = JSON.parse(data);
        $('#kdag').val(hss[0].kode_agent);
        $('#nmag').val(hss[0].nama);
        $('#tlag').val(hss[0].telp);
        $('#alag').val(hss[0].alamat);
        $('#idag').val(hss[0].id_agent);
        var cek = '';
  ...
  function detaild(id) {
    ...
    $.ajax({
      type:"GET",
      url:"<?php echo base_url(); ?>agent/detail/"+id,
      beforeSend : function () {
        swal({
          title  : 'Menunggu',
          html   : 'Memproses Data',
      ...
      success  : function (data) {
        ...
        var lsd = "<table style='width:100%; text-align: left;'>"+
                    "<tr>"+
                      "<td style='width:25%;'><b>Kode Agent</b></td><td>:</td><td>"+hss[0].kode_agent+"</td>"+
                    "</tr>"+
                    "<tr>"+
                      "<td style='width:25%;'><b>Nama Agent</b></td><td>:</td><td>"+hss[0].nama+"</td>"+
                    "</tr>"+
                    "<tr>"+
  ...
  function deletedel(id) {
    swal({
      title       : 'Konfirmasi',
      text        : 'Yakin akan Menghapus Data Agent',
      type        : 'warning',
      buttonsStyling      : false,
      confirmButtonClass  : "btn btn-primary",
      cancelButtonClass   : "btn btn-warning mr-3",
      ...
      if (result.value) {
        $.ajax({
          type:"GET",
          url:"<?php echo base_url(); ?>agent/remove/"+id,
          beforeSend : function () {
            swal({
              title  : 'Menunggu',
              html   : 'Memproses Data',
          ...
          success  : function (data) {
            swal({
              title             : "Berhasil",
              text              : "Data Agent telah di Hapus",
              type              : 'success',
              showConfirmButton : false,
              timer               : 3000
            });
