...
<?php if ($role['read'] == true || $role == null): ?>
  <div class="row">
    ...
    <div class="col-md-5">
      <div class="card shadow">
        <form id="colectbspr" method="post">
          <div class="card-body">
            ...
            <div class="form-group">
              <label>Kode Business Partner</label>
              <input type="text" name="kdbp" id="kdbp" class="form-control" required placeholder="Kode Business Partner" readonly/>
            </div>
            <div class="form-group">
              <label>Nama<b style="color:red;">*</b></label>
              <input type="hidden" name="idbp" id="idbp" value="">
              <input type="text" name="nmbp" id="nmbp" class="form-control" required placeholder="Nama"/>
            </div>
            <div class="form-group">
              <label>No. Telepon<b style="color:red;">*</b></label>
              <input type="text" name="tlbp" id="tlbp" class="form-control numeric" required placeholder="Telepon"/>
            </div>
            ...
            <div class="form-group">
              ...
              <textarea name="albp" id="albp" class="form-control" required placeholder="Alamat" rows="8" cols="80"></textarea>
            </div>
...
<script type="text/javascript">
  ...
  function getkode() {
    $.ajax({
      type:"GET",
      url:"<?php echo base_url(); ?>business_partner/business_partner_kode",
  ...
  $(document).ready(function () {
    ...
    table_bp_ro = $('#datatable_ro').DataTable({
      "responsive": true,
      "processing" : true,
      "serverSide" : true,
      "order" : [],
      "ajax" : {
        "url" : "<?php echo base_url(); ?>business_partner/ajaxdata/"+act,
        "type" : "POST",
        "data" : { "tipen":'t' }
      },
      "columnDefs" : [{
    ...
    table_bp_ru = $('#datatable_ru').DataTable({
      "responsive": true,
      "processing" : true,
      "serverSide" : true,
      "order" : [],
      ...
      swal({
        ...
        if (result.value) {
          ...
          if (bpid == '') {
            $.ajax({
              type:"POST",
              url:"<?php echo base_url(); ?>business_partner/add",
              beforeSend : function () {
                swal({
                  title  : 'Menunggu',
                  html   : 'Memproses Data',
            ...
            $.ajax({
              type:"POST",
              url:"<?php echo base_url(); ?>business_partner/edit/"+bpid,
              beforeSend : function () {
                swal({
                  title  : 'Menunggu',
                  html   : 'Memproses Data',
          ...
          table_bp_ro.ajax.reload();
          table_bp_ru.ajax.reload();
          table_bp_ro.columns.adjust().draw();
          table_bp_ru.columns.adjust().draw();
        } else if (result.dismiss === swal.DismissReason.cancel) {
          swal({
            title               : "Batal",
            text                : 'Anda membatalkan Penginputan Data Business Partner',
            buttonsStyling      : false,
            confirmButtonClass  : "btn btn-primary",
            type                : 'error',
            showConfirmButton   : false,
  ...
  function ubahubah(id) {
    ...
    $.ajax({
      type:"GET",
      url:"<?php echo base_url(); ?>business_partner/show/"+id,
      success  : function (data) {
        var hss = JSON.parse(data);
        $('#kdbp').val(hss[0].kode_business_partner);
        $('#nmbp').val(hss[0].nama);
        $('#tlbp').val(hss[0].telp);
        $('#albp').val(hss[0].alamat);
        $('#idbp').val(hss[0].id_business_partner);
        var cek = '';
  ...
  function detaild(id) {
    ...
    $.ajax({
      type:"GET",
      url:"<?php echo base_url(); ?>business_partner/detail/"+id,
      beforeSend : function () {
        swal({
          title  : 'Menunggu',
          html   : 'Memproses Data',
      ...
      success  : function (data) {
        ...
        var lsd = "<table style='width:100%; text-align: left;'>"+
                    "<tr>"+
                      "<td style='width:33%;'><b>Kode Business Partner</b></td><td>:</td><td>"+hss[0].kode_business_partner+"</td>"+
                    "</tr>"+
                    "<tr>"+
                      "<td style='width:33%;'><b>Nama Business Partner</b></td><td>:</td><td>"+hss[0].nama+"</td>"+
                    "</tr>"+
                    "<tr>"+
                      "<td style='width:33%;'><b>Telpon</b></td><td>:</td><td>"+hss[0].telp+"</td>"+
                    "</tr>"+
                    "<tr>"+
                      "<td style='width:33%;'><b>Negara</b></td><td>:</td><td>"+(hss[0].negara == null?'-':hss[0].negara)+"</td>"+
                    "</tr>"+
                    "<tr>"+
                      "<td style='width:33%;'><b>Provinsi</b></td><td>:</td><td>"+(hss[0].provinsi == null?'-':hss[0].provinsi)+"</td>"+
                    "</tr>"+
                    "<tr>"+
                      "<td style='width:33%;'><b>kabupaten/Kota</b></td><td>:</td><td>"+(hss[0].kota == null?'-':hss[0].kota)+"</td>"+
                    "</tr>"+
                    "<tr>"+
                      "<td style='width:33%;'><b>Kecamatan</b></td><td>:</td><td>"+(hss[0].kecamatan == null?'-':hss[0].kecamatan)+"</td>"+
                    "</tr>"+
                    "<tr>"+
                      "<td style='width:33%;'><b>Kelurahan/Desa</b></td><td>:</td><td>"+(hss[0].desa == null?'-':hss[0].desa)+"</td>"+
                    "</tr>"+
                    "<tr>"+
                      "<td style='width:33%;'><b>Alamat Lengkap<b></td><td>:</td><td>"+(hss[0].alamat == null?'-':hss[0].alamat)+"</td>"+
                    "</tr>"+
                  "</table>";
        swal({
          title             : "Detail Business Partner",
          html              : lsd,
  ...
  function deletedel(id) {
    swal({
      title       : 'Konfirmasi',
      text        : 'Yakin akan Menghapus Data Business Partner',
      type        : 'warning',
      buttonsStyling      : false,
      confirmButtonClass  : "btn btn-primary",
      cancelButtonClass   : "btn btn-warning mr-3",
      ...
      if (result.value) {
        $.ajax({
          type:"GET",
          url:"<?php echo base_url(); ?>business_partner/remove/"+id,
          beforeSend : function () {
            swal({
              title  : 'Menunggu',
              html   : 'Memproses Data',
          ...
          success  : function (data) {
            swal({
              title             : "Berhasil",
              text              : "Data Business Partner Adjuster telah di Hapus",
              type              : 'success',
              showConfirmButton : false,
              timer               : 3000
            });
