...
class M_entry_sppa extends CI_Model
{
    ...
    {
        $this->db->select('m.no_mop, a.nama_asuransi, c.cob, l.lob, l.punya_ahli_waris, l.premi_ditentukan');
        $this->db->from('mop m');
    ...
    // 10-11-2021
    public function cari_list_premi($id_mop)
    {
    ...
    {
        $this->db->select('ma.nama_asuransi, ml.lob, cl.cob, tpa.premi, pt.*, pm.nama as payment_method, pm.id as id_payment_method, p.pekerjaan, mm.method, mm.id as id_method, ml.punya_ahli_waris, ta.no_polis, ta.tgl_awal_polis, ta.tgl_akhir_polis, ta.id_produk_asuransi, ta.id_mop, mp.id_insured, ta.*, n.nama_nasabah, mp.no_reff_mop, mp.no_mop');
        ...
        $this->db->join('m_asuransi ma', 'ma.id_asuransi = tpa.id_asuransi', 'inner');
        $this->db->join('mop mp', 'mp.id_mop = ta.id_mop', 'left');
        $this->db->join('m_nasabah n', 'n.id_nasabah = mp.id_insured', 'left');
        $this->db->join('relasi_cob_lob rc', 'rc.id_relasi_cob_lob = mp.id_relasi_cob_lob', 'left');
        $this->db->join('m_lob ml', 'ml.id_lob = rc.id_lob', 'left');
        $this->db->join('m_cob cl', 'cl.id_cob = rc.id_cob', 'left');
    ...
    // 16-11-2021
    public function get_list_ahli_waris($id_sppa)
    {
        $this->db->select('ma.*, h.status');
        $this->db->from('m_ahli_waris ma');
        $this->db->join('m_hubungan_klg h', 'h.id = ma.hubungan', 'inner');
        $this->db->where('ma.id_tr_sppa_quotation', $id_sppa);
